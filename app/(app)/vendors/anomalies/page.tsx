...
import { listAllVendorAnomalies } from "@/lib/vendors-data";
...
export default async function VendorAnomaliesPage() {
  const anomalies = await listAllVendorAnomalies();

  return (
    <>
      <Topbar pageTitle="Vendor anomalies" />
        ...
        <div className="mx-auto max-w-[1400px] space-y-4 px-6 py-6">
          ...
          <section className="rounded-[var(--radius-card)] border border-[var(--border)] bg-white px-5 py-4 shadow-[var(--shadow-card)]">
            <h1 className="text-[20px] font-semibold text-[var(--ink-900)]">
              Vendor invoice anomalies — {anomalies.length}
            </h1>
            <p className="mt-0.5 text-[12px] text-[var(--ink-500)]">
              Missing lines · duplicates · price drift · deduction mismatch · invalid GSTIN
            </p>
          </section>
